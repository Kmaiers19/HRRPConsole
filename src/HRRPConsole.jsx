...
function generateInternal(seed, mode) {
 ...
 const characteristicLoad = snhEffect + largeEffect + mediumEffect + teachingEffect + smal const biasTerm = -0.030;
 const conditions = {};
 let totalExcess = 0;
 ...
 const discharges = Math.floor(25 + rng() * (cond === 'HF' || cond === 'PN' ? 400 : 180) const cases = Math.floor(discharges * (0.15 + rng() * 0.08));
 conditions[cond] = {
...
function declaredThreshold(hospitals) {
 const penalized = hospitals.filter(h => h.paymentAdjustment > 0).map(h => h.paymentAdjustme if (penalized.length === 0) return 0.0045;
...
function Panel({ title, subtitle, tag, children, style }) {
 ...
 <span style={{ color: C.textDim, fontSize: 10, letterSpacing: '0.05em' }}>{title}</ {subtitle && <span style={{ color: C.textMuted, fontSize: 10 }}>{subtitle}</span>}
 </div>
 {tag && <span style={{ color: C.textMuted, fontSize: 9, letterSpacing: '0.05em' }}>{t </div>
 <div>{children}</div>
 </div>
 );
}
...
export default function HRRPConsole() {
 ...
 const nonSnhHigh = nonSnh.filter(h => h.paymentAdjustment >= highPenaltyThreshold).length return {
 snhN: snh.length,
 ...
 { group: 'non-SNH', achieved: pct(nonSnh), target: 0.30, n: nonSnh.length { group: 'Large', achieved: pct(large), target: 0.40, n: large.length  { group: 'Small', achieved: pct(small), target: 0.28, n: small.length  { group: 'Teaching', achieved: pct(teach), target: 0.44, n: teach.length  { group: 'Non-teaching', achieved: pct(nonTeach),target: 0.33, n: nonTeach.leng { group: 'Overall penalty rate', achieved: filtered.length ? penalized / filtered.lengt ];
 }, [filtered, highPenaltyThreshold]);
 ...
 let verdict, color;
 if (maxMissPp <= 3) {
 verdict = `Synthetic generator hits J&J 2013 published rates within ${maxMissPp.toFixed color = C.green;
 } else if (maxMissPp <= 8) {
 verdict = `Synthetic generator close to J&J 2013 published rates. Max group deviation $ color = C.amber;
 } else {
 verdict = `Synthetic generator deviates from J&J 2013 by up to ${maxMissPp.toFixed(1)}p color = C.red;
 }
 ...
 const sPct = snh.length ? snh.filter(h => h.paymentAdjustment >= t).length / snh.length const nPct = nonSnh.length ? nonSnh.filter(h => h.paymentAdjustment >= t).length / nonS points.push({ t: t * 100, gap: (sPct - nPct) * 100 });
 }
 return points;
 }, [filtered]);
 const kpis = useMemo(() => {
 ...
 if (conditionFilter !== 'ALL') return acc + (h.conditions[conditionFilter]?.err ||  return acc + (h.meanERR || 0);
 }, 0) / filtered.length
 : 0;
 ...
 if (typeof av === 'string') return sortDir === 'asc' ? av.localeCompare(bv) : bv.locale return sortDir === 'asc' ? av - bv : bv - av;
 });
 ...
 <div>
 <div style={{ color: C.amber, fontSize: 11, fontWeight: 'bold', marginBottom: 6 } What this demonstrates
 </div>
 ...
 <div>
 <div style={{ color: C.amber, fontSize: 11, fontWeight: 'bold', marginBottom: 6 } What this does not claim
 </div>
 <div style={{ color: C.text, fontSize: 11, lineHeight: 1.55 }}>
 ...
 <div>
 <div style={{ color: C.amber, fontSize: 11, fontWeight: 'bold', marginBottom: 6 } Next step
 </div>
 <div style={{ color: C.text, fontSize: 11, lineHeight: 1.55 }}>
 ...
 {/* ============== TOP BAR (chrome stripped: no clock, no LAT, no CONN) ==============  <div style={{
 display: 'flex',
 justifyContent: 'space-between',
 alignItems: 'center',
 padding: '6px 12px',
 ...
 DGP={dgp === 'loaded' ? 'LOADED · group effects modeled' : 'NULL · no group effec </span>
 </div>
 </div>
 {/* ============== DGP TOGGLE ============== */}
 <div style={{
 ...
 <span style={{ color: C.amber, fontSize: 10, fontWeight: 'bold', letterSpacing: '0.08 DGP
 </span>
 <button
 onClick={() => setDgp('null')}
 style={{
 ...
 (at threshold {(highPenaltyThreshold * 100).toFixed(2)}%; declared: {(DECLARED_TH </span>
 </div>
 )}
 {/* ============== STAT STRIP ============== */}
 <div style={{
 ...
 *
 ...
 <span style={{ color: C.textMuted, fontSize: 10, fontStyle: 'italic' }}>
 declared to median paymentAdjustment among penalized hospitals; slider for sensitiv </span>
 </div>
 ...
 <BarChart data={conditionBreakdown} margin={{ top: 10, right: 20, left: 10, bot <CartesianGrid stroke={C.border} strokeDasharray="2 4" vertical={false} />
 <XAxis dataKey="condition" tick={{ fill: C.textDim, fontSize: 10, fontFamily: <YAxis tickFormatter={v => `${(v * 100).toFixed(0)}%`} tick={{ fill: C.textDi <Tooltip
 contentStyle={{ background: C.bg, border: `1px solid ${C.borderBright}`, bo itemStyle={{ color: C.text }}
 ...
 {conditionBreakdown.map(c => `${c.condition}:${(c.penalizedPct * 100).toFixed(0 </div>
 </div>
 </Panel>
 {/* SCATTER */}
 <Panel
 ...
 <div style={{
 padding: '8px 12px',
 background: dgp === 'loaded' ? 'rgba(232, 169, 60, 0.08)' : 'rgba(82, 148, 226, 0 borderBottom: `1px solid ${C.border}`,
 color: C.textDim,
 fontSize: 10,
 fontStyle: 'italic',
 lineHeight: 1.5,
 ...
 <XAxis type="number" dataKey="x" tickFormatter={v => `${v.toFixed(0)}%`} tick <YAxis type="number" dataKey="y" tick={{ fill: C.textDim, fontSize: 10, fontF <ZAxis range={[20, 20]} />
 ...
 />
 <Scatter data={scatterData} fill={dgp === 'loaded' ? C.amber : C.blue} fillOp </ScatterChart>
 </ResponsiveContainer>
 <div style={{ padding: '4px 12px 10px', color: C.textDim, fontSize: 10, lineHeigh {regression && (
 <>
 ...
 <br/>
 SNH high-pen: <span style={{ color: C.amber }}>{fmtPct(snhSplit.snhHighPct, vs non-SNH: <span style={{ color: C.text }}>{fmtPct(snhSplit.nonSnhHighPct, {' · '}
 ...
 tag="v0.5.1"
 >
 <div style={{ padding: '12px 14px', fontSize: 11, lineHeight: 1.55, color: C.text } <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.redDim <div style={{ color: C.redDim, fontSize: 10, fontWeight: 'bold' }}>v0.1.0 — ret <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 ...
 <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.blue}` <div style={{ color: C.blue, fontSize: 10, fontWeight: 'bold' }}>v0.2.0 — null  <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 ...
 <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.amber} <div style={{ color: C.amber, fontSize: 10, fontWeight: 'bold' }}>v0.3 — DGP to <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 ...
 <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.greenD <div style={{ color: C.greenDim, fontSize: 10, fontWeight: 'bold' }}>v0.4.0 — c <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 ...
 <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.greenD <div style={{ color: C.greenDim, fontSize: 10, fontWeight: 'bold' }}>v0.5.0 — r <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 ...
 <div style={{ marginBottom: 10, paddingLeft: 8, borderLeft: `2px solid ${C.green} <div style={{ color: C.green, fontSize: 10, fontWeight: 'bold' }}>v0.5.1 — term <div style={{ color: C.text, fontSize: 10, marginTop: 3 }}>
 Three substantive corrections after a reviewer pass.{' '}
 <span style={{ fontWeight: 'bold' }}>(1) "Pre-registered" → "declared":</span pre-registration is a methodological commitment that requires
 ...
 three-condition era.{' '}
 <span style={{ fontWeight: 'bold' }}>(3) Threshold language softened:</span>{ removed "fixed before observation" from threshold rule
 ...
 <th style={{ textAlign: 'left', padding: '6px 8px', color: C.textDim, fontWe <th style={{ textAlign: 'right', padding: '6px 8px', color: C.textDim, fontWe <th style={{ textAlign: 'right', padding: '6px 8px', color: C.textDim, fontWe <th style={{ textAlign: 'right', padding: '6px 8px', color: C.textDim, fontWe <th style={{ textAlign: 'right', padding: '6px 8px', color: C.textDim, fontWe <th style={{ textAlign: 'left', padding: '6px 8px', color: C.textDim, fontWe </tr>
 </thead>
 <tbody>
 ...
 const targetApplicable = dgp === 'loaded' || row.group === 'Overall penalty r return (
 <tr key={row.group} style={{
 borderBottom: `1px solid ${C.border}`,
 background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)',
 }}>
 <td style={{ padding: '6px 8px', color: C.text }}>{row.group}</td>
 <td style={{ padding: '6px 8px', textAlign: 'right', color: C.textDim }}> <td style={{ padding: '6px 8px', textAlign: 'right', color: targetApplica {(row.target * 100).toFixed(0)}%
 </td>
 ...
 <span style={{ color: C.textMuted, fontSize: 10 }}>n/a under null DGP )}
 </td>
 </tr>
 );
 })}
 ...
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Target Joynt &amp; Jha 2013 JAMA 309(4):342–343 Table 1.{' '}
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Δ:</sp achieved minus target in percentage points; bands green ≤3pp, amber 3–8pp, red  </div>
 <div style={{ marginBottom: 6 }}>
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Scope  The 2013 paper analyzed first-year HRRP penalties, when the program covered the
 ...
 conditions but the calibration target rates come from the original three-condit era of the program.
 </div>
 <div style={{ marginBottom: 6 }}>
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Thresh
 ...
 ±50% band. Median is conventional; other rules (mean, top-quartile, fixed-perce would give different cutoffs but should not change the qualitative SNH-vs-non-S comparison.
 </div>
 <div style={{ marginBottom: 6 }}>
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Sampli SNH n=71 means the achieved SNH rate has a standard error of roughly ±5–6pp fro binomial sampling alone, before any modeling error. Group-level deltas within t range should be read as noise, not signal.
 </div>
 <div>
 <span style={{ color: C.text, fontWeight: 'bold', fontStyle: 'normal' }}>Per-DG the declared threshold is computed from the LOADED DGP. NULL DGP renders the
 ...
 tag={`band: ${(DECLARED_THRESHOLD * 50).toFixed(2)}% – ${(DECLARED_THRESHOLD * 150).t style={{ borderTop: 'none' }}
 >
 <div style={{ padding: '12px 16px' }}>
 <div style={{ color: C.textMuted, fontSize: 10, marginBottom: 8, lineHeight: 1.5 }} Y axis: percentage-point gap between SNH and non-SNH high-penalty
 ...
 <LineChart data={thresholdSweep} margin={{ top: 10, right: 30, left: 0, bottom: 5 <CartesianGrid stroke={C.border} strokeDasharray="2 4" />
 <XAxis
 dataKey="t"
 ...
 domain={['auto', 'auto']}
 />
 <ReferenceLine y={14} stroke={C.green} strokeDasharray="4 2" label={{ value: 'J <ReferenceLine x={DECLARED_THRESHOLD * 100} stroke={C.blue} strokeDasharray="4  <Tooltip
 contentStyle={{ background: C.bg, border: `1px solid ${C.borderBright}`, bord itemStyle={{ color: C.text }}
 labelStyle={{ color: C.textDim }}
 formatter={(v, n) => [`${v.toFixed(1)}pp`, 'SNH gap']}
 labelFormatter={(v) => `threshold ${v.toFixed(3)}%`}
 />
 <Line type="monotone" dataKey="gap" stroke={C.amber} strokeWidth={2} dot={false </LineChart>
 </ResponsiveContainer>
 </div>
 </Panel>
 ...
 <div>
 <div style={{ color: C.amber, fontSize: 10, marginBottom: 4, letterSpacing: '0.08 A · CALIBRATION SOURCE
 </div>
 <div style={{ color: C.textDim, fontSize: 10, fontStyle: 'italic', marginBottom:  The published rates this dashboard's LOADED DGP is tuned to.
 </div>
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Joynt &amp; Jh <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Gilman et al.  <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Chaiyachati, Q <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div>
 <div style={{ color: C.blue, fontSize: 10, marginBottom: 4, letterSpacing: '0.08e B · MORTALITY DEBATE
 </div>
 <div style={{ color: C.textDim, fontSize: 10, fontStyle: 'italic', marginBottom:  Did HRRP cause harm? The literature disagrees.
 </div>
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Wadhera et al. <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Khera et al. 2 <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div>
 <div style={{ color: C.yellow, fontSize: 10, marginBottom: 4, letterSpacing: '0.0 C · MEASUREMENT CRITIQUE
 </div>
 <div style={{ color: C.textDim, fontSize: 10, fontStyle: 'italic', marginBottom:  Are the reported reductions real, or partly an artifact of how ERR is construct </div>
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Ibrahim et al. <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <div style={{ marginBottom: 12, paddingLeft: 8, borderLeft: `2px solid ${C.border <div style={{ color: C.text, fontSize: 10, fontWeight: 'bold' }}>Ody, Msall, Da <div style={{ color: C.textDim, fontSize: 10, marginBottom: 3 }}>
 ...
 <thead style={{ position: 'sticky', top: 0, background: C.bgPanelAlt, zIndex: 1 } <tr>
 {[
 ...
 style={{
 background: isSelected ? C.bgPanelAlt : (i % 2 === 0 ? 'transparent'  cursor: 'pointer',
 borderBottom: `1px solid ${C.border}`,
 }}
 >
 ...
 <td style={{ padding: '5px 8px', textAlign: 'right', color: C.textDim } <td style={{ padding: '5px 8px', textAlign: 'right', color: C.text }}>
 ...
 <td style={{ padding: '5px 8px', textAlign: 'right', color: h.condition {h.conditionsPenalized}/6
 </td>
 <td style={{ padding: '5px 8px', textAlign: 'right', color: h.paymentAd {(h.paymentAdjustment * 100).toFixed(2)}
 </td>
 <td style={{ padding: '5px 8px', textAlign: 'right', color: h.penaltyDo {h.penaltyDollars > 0 ? fmtMoney(h.penaltyDollars) : '—'}
 </td>
 </tr>
 {isSelected && (
 <tr style={{ background: C.bgPanelAlt, borderBottom: `1px solid ${C.bor <td colSpan={10} style={{ padding: '12px 16px' }}>
 <div style={{ color: C.textMuted, fontSize: 10, marginBottom: 8 }}>
 <span style={{ color: C.blue }}>&gt; </span>
 expand {h.ccn} · cond_detail · SNH={h.isSNH ? 'yes' : 'no'} · DGP </div>
 <div style={{
 display: 'grid',
 ...
 <div style={{ color: C.textMuted, fontSize: 10 }}>{cond}< <div style={{ color: C.textMuted, fontSize: 11, marginTop </div>
 );
 }
 ...
 <div style={{ color: C.textMuted, fontSize: 10, marginTop:  n={c.discharges}
 </div>
 </div>
 );
 })}
 ...
 <span style={{ color: C.textMuted, fontStyle: 'italic' }}>(arbitrary cutoffs, demon </div>
